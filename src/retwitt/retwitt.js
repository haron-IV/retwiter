...
const {
  ...
  confirmRetwitt,
  likeTwitt,
  checkAuthorBeforeShare,
} = require("./page-actions");
const { twittLink } = require("./create-twitt-link");
const { addHashTags } = require("../add-comment/add-hashtags");

const {
  waitMinsAfterGoToHome,
  waitMinsAfterRetwitt,
  waitMinsAfterSelectingAlreadyRetwittedPost,
} = getAppConfig();
...
const retwitt = async (page) => {
  ...
  if (twittToShareLink) {
    if (!(await wasTwittShared(twittToShareLink)))
      await shareTwittAndRepeat(page, twittToShareLink);
